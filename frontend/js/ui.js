...
const UI = (() => {
  ...
  // `ico` é o nome de um ícone do módulo Icons (icons.js).
  const NAV = [
    { id: "dashboard",      label: "Dashboard",     ico: "dashboard",    href: "index.html" },
    { id: "lancamentos",    label: "Lançamentos",   ico: "transactions", href: "pages/lancamentos.html" },
    { id: "planejamento",   label: "Planejamento",  ico: "calendar",     href: "pages/planejamento.html" },
    { id: "cartoes",        label: "Cartões",       ico: "card",         href: "pages/cartoes.html" },
    { id: "metas",          label: "Metas",         ico: "target",       href: "pages/metas.html" },
    { id: "alertas",        label: "Alertas",       ico: "bell",         href: "pages/alertas.html" },
    { id: "relatorios",     label: "Relatórios",    ico: "chart",        href: "pages/relatorios.html" },
    { id: "ia",             label: "IA Financeira", ico: "ai",           href: "pages/ia.html" },
    { id: "configuracoes",  label: "Configurações", ico: "settings",     href: "pages/configuracoes.html" },
  ];
  ...
  function mountLayout({ page, title, topbarExtra = "" }) {
    ...
    const links = NAV.map((n) => `
      <a class="nav-link ${n.id === page ? "active" : ""}" href="${prefix}${n.href}">
        <span class="ico">${ic(n.ico)}</span><span>${n.label}</span>
    ...
    if (sidebar) {
      ...
      sidebar.innerHTML = `
        <div class="brand"><span class="brand-logo">${ic("brand")}</span><span>Finance</span></div>
        <nav>${links}</nav>
    ...
    if (topbar) {
      ...
      topbar.innerHTML = `
        <button class="hamburger" id="hamburger" aria-label="Menu">${ic("menu")}</button>
  ...
  /**
   * Liga uma barra de abas aos seus painéis. Espera o HTML:
   *   <div class="tabs">
   ...
   *
   * `onChange(key)` (opcional) roda ao trocar de aba (ex: carregar dados sob demanda).
   * Retorna uma função para ativar uma aba programaticamente.
   */
  function wireTabs(tabsEl, { onChange } = {}) {
    if (!tabsEl) return () => {};
    const scope = tabsEl.parentElement || document;
    const ativar = (key) => {
    ...
    for (const a of accounts) {
      ...
      card.innerHTML = `
        <div class="account-top">
          <span class="account-icon">${a.icon ? escapeHtml(a.icon) : ic("wallet")}</span>
        ...
        <div class="account-flow">
          <span class="up">${ic("arrow-up", { size: 13 })} ${formatarMoeda(a.income)}</span>
          <span class="down">${ic("arrow-down", { size: 13 })} ${formatarMoeda(a.expense)}</span>
        </div>`;
  ...
  function renderCardCards(cards, container) {
    ...
    for (const c of cards) {
      ...
      card.innerHTML = `
        <div class="cc-top">
          <span class="cc-name">${ic("card", { size: 16 })}${escapeHtml(c.name)}</span>
  ...
  function renderGoalCards(goals, container) {
    ...
    for (const g of goals) {
      ...
      card.innerHTML = `
        <div class="cc-top">
          <span class="cc-name">${ic("target", { size: 16 })}${escapeHtml(g.name)}</span>
          ${g.status === "atingida" ? `<span class="badge badge-ok">${ic("check", { size: 12 })}atingida</span>` : g.exceeded ? `<span class="badge badge-danger">estourou</span>` : ""}
        </div>
  ...
  function renderAlerts(alerts, container, { onChange } = {}) {
    ...
    // Ícone por severidade (substitui o emoji que vinha do backend).
    const icoPorSev = { danger: "alert-triangle", warning: "alert-triangle", info: "info" };
    for (const a of visiveis) {
      ...
      card.innerHTML = `
        <span class="alert-ico" style="color:var(--${cor})">${ic(icoPorSev[a.severity] || "bell")}</span>
        <div class="alert-body">
          <h3>${escapeHtml(a.title)}</h3>
          <p>${escapeHtml(a.message)}</p>
        </div>
        <button class="btn btn-ghost btn-sm" data-read="${escapeHtml(a.key)}">${ic("check", { size: 15 })} Lido</button>`;
      container.appendChild(card);
    }
  ...
  function renderTransactionsTable(transactions, opts = {}) {
    ...
    for (const t of transactions) {
      ...
      const acoes = withActions ? `
        <td class="num">
          ${onEdit ? `<button class="icon-btn" data-edit="${t.id}" title="Editar" aria-label="Editar">${ic("edit")}</button>` : ""}
          ${onDelete ? `<button class="icon-btn danger" data-del="${t.id}" title="Excluir" aria-label="Excluir">${ic("trash")}</button>` : ""}
        </td>` : "";
  ...
  return {
    mountLayout, wireTabs, showMessage, showStatus, clearStatus, fillSelect,
