...
from app.api.dependencies import get_advisor_service, get_report_service
from app.api.schemas import HealthScoreOut, MonthlySummaryOut, TransactionOut
...
@router.post("/health-score", response_model=HealthScoreOut)
def health_score(
    year: int = Query(default=date.today().year, description="Ano."),
    month: int = Query(default=date.today().month, ge=1, le=12, description="Mês (1-12)."),
    service: FinancialAdvisorService = Depends(get_advisor_service),
):
    """Score de Saúde Financeira (0-100) + previsão de fim de mês + insights.

    O score é determinístico (média ponderada de 5 sub-métricas). Com
    GROQ_API_KEY configurada, acrescenta uma interpretação em linguagem
    natural. O resultado é cacheado e só recalcula quando o mês muda de
    transações — então é barato chamar de novo.
    """
    return HealthScoreOut(**service.health_score(year, month))
